...
def compute_effective_demand(freq, repriced, orig_price, new_price, cross_price):
    ...
    for B, qB in freq.items():
        ...
        E_D[B] = E
        # Demand change bounded at -100% minimum
...
if __name__ == "__main__":
    random.seed(42)
    # load data
    with open("fruithut_utility.txt") as f:
        raw = f.readlines()
    ...
    cross_price = load_cross_price("distance_matrix_fruithut.csv",
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(parsed)):
        ...
        for alpha in alpha_values:
            cross_price = load_cross_price("distance_matrix_fruithut.csv", alpha, defaults['beta'])
            for approach in approaches:
                start = time.time()
                L, price_map, demand_ch, E_D_map, freq = mine_itemsets(
                    train, cross_price, approach,
        ...
        for beta in beta_values:
            cross_price = load_cross_price("distance_matrix_fruithut.csv", defaults['alpha'], beta)
            for approach in approaches:
                start = time.time()
                L, price_map, demand_ch, E_D_map, freq = mine_itemsets(
                    train, cross_price, approach,
